...
inline void avx2_dp(float* x, float* y, float* ret_dp, int size) {
	// dot product x, y => ret_dp
	// sum x => ret_sum
	// for (int i = 0; i < size; i++) {
	// 	*ret_dp += x[i] * y[i];
...
inline void avx2_sum(float* x, float* ret_sum, int size) {
	// sum of an array
	// for (int i = 0; i < size; i++) {
	// 	*ret_sum += x[i];
	// }
...
void getPairwiseAffinity(float *d, int n_samples, float perplexity, float *p) {
	...
	// thandle t1, t2, t3;
	// t1 = create_timer("A"), t2 = create_timer("B");
