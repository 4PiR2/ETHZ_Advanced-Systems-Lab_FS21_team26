...
int ntimers = 0;
const int NB1=3, NB2=8, MU=4, NU=4, KU=8;
...
static void run_baseline(int n_samples, int d_out, int d_in, int rep, float eta, float alpha, float perplexity, int n_iter, std::string file_in, std::string file_out) {
	...
	for (int r = 0; r < rep; r++) {
		...
		memcpy(dy, y, d_out*n_samples*sizeof(float));
		mat_transpose(y_trans, y, n_samples, d_out);
		//memset(t, 0.0, n_samples*n_samples*sizeof(float)); 
		mat_clear(t, n_samples, n_samples);
		//printf("%f %f\n", y[1799],dy[1799]);
		start(t1);
		...
		start(t2);
		getLowDimResult(y, y_trans, dy, grad_cy, p, t, n_samples, d_out, alpha, eta, n_iter, NB1, NB2, MU, NU, KU);
		stop(t2);
...
int main(int argc, char *argv[]) {
	int n_samples = 5000, d_in = 784, d_out = 8, n_iter = 100 /*,n_iter_ex = 250*/ /*,seed = 0*/;
